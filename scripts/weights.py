"""Persist operational EX-CPI reconstruction shares, vintaged like observations.

These are not the published basket. MM23 publishes each exclusion aggregate
and its complement as annual weights in parts per thousand, and those land
verbatim in ``original_weights`` with their regime year. This module stores
the derived system actually used to rebuild an aggregate: shares in [0, 1]
normalised within each aggregate/complement pair, applied to price ratios
rebased on the previous December.

Keeping both is not duplication. ``original_weights`` answers "what did ONS
publish", ``weights`` answers "what reconstructs the index", and measured
against live MM23 those are different questions: applying the published
parts-per-thousand directly to index levels leaves residuals up to 1.45
index points that scale with the size of the complement, while the shares
here close to 0.18 uniformly, which is the rounding floor of a file
...
def _write_batches(
    ...
    if not rows:
        return
    logger.info(
        "Operational weights %s: writing %d rows in batches of %d", operation, len(rows), BATCH_SIZE
    )
    for start in range(0, len(rows), BATCH_SIZE):
        ...
        logger.info(
            "Operational weights %s progress: %d/%d rows",
            operation,
...
def upsert_weights(
    ...
    """Write operational shares idempotently and return ``(new, new_vintages)``.

    A share outside [0, 1] means published basket points reached this table, so
    it is rejected rather than stored next to genuine shares.
    """
    ...
    for row in rows:
        ...
        if not 0.0 <= weight <= 1.0:
            raise ValueError(
                f"{row['series_id']} at {row['reference_date']}: operational share "
                f"{weight!r} is outside [0, 1]; parts per thousand belong in "
                "original_weights"
            )
        incoming.append(
            {
    ...
    logger.info("Operational weights upsert: evaluating %d incoming weights", len(incoming))
    if not incoming:
        logger.info("No operational weight rows to upsert")
        return 0, 0
    ...
    logger.info(
        "Operational weights upsert: new=%d new_vintages=%d same_day_updates=%d",
        new_rows,
        new_vintages,
        len(updates),
    )
